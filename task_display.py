...
def display_tasks():
    ...
    if table_exists:
        ...
        # Destroy the existing add button
        existing_add_button = info_frame.children.get("add_button")
        if existing_add_button:
            existing_add_button.destroy()

        # Create a new add button
        add_button = Button(info_frame, text="Add", command=add, name="add_button")
        add_button.grid(padx=5, sticky="W")
...
def back():
    ...
    # Destroy root window
    root.destroy()
    # Subprocess to launch new process running home page
...
# Get the name entered by the user and capitalize the first letter
CAPITALISED_NAME = NAME_VALUE.title()

# Create and set the welcome message text variable
welcome_label = Label(title_frame, text=f"Welcome, {CAPITALISED_NAME}", font=("Arial", 20))
